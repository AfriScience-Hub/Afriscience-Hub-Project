...
import { Search, Filter, Plus, MoreHorizontal, Target } from 'lucide-react';

const mockCentres = [
  { id: 1, name: 'Nairobi Science Centre', field: 'General Science', country: 'Kenya', status: 'Active', programs: 12 },
  { id: 2, name: 'Lagos Innovation Hub', field: 'Technology', country: 'Nigeria', status: 'Active', programs: 8 },
  { id: 3, name: 'Cape Town Research Lab', field: 'Biotechnology', country: 'South Africa', status: 'Pending', programs: 5 },
];

export default function SpecialCentresPage() {
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-xs text-neutral-gray-medium mb-1">
            ...
            <span className="text-neutral-black font-medium">Special Centres</span>
          </div>
          <h1 className="text-2xl font-bold text-neutral-black flex items-center gap-2">
            <Target className="h-6 w-6 text-green-600" />
            Special Centres
          </h1>
          <p className="text-sm text-neutral-gray-dark mt-1">Manage specialist centres and research facilities.</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="rounded-xl border border-neutral-gray-light bg-white p-5 shadow-sm">
          <p className="text-xs text-neutral-gray-medium">Total Centres</p>
          <p className="text-2xl font-bold text-neutral-black mt-1">892</p>
        </div>
        <div className="rounded-xl border border-neutral-gray-light bg-white p-5 shadow-sm">
          <p className="text-xs text-neutral-gray-medium">Active</p>
          <p className="text-2xl font-bold text-neutral-black mt-1">834</p>
        </div>
        <div className="rounded-xl border border-neutral-gray-light bg-white p-5 shadow-sm">
          <p className="text-xs text-neutral-gray-medium">Pending Review</p>
          <p className="text-2xl font-bold text-neutral-black mt-1">58</p>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1 max-w-md">
          ...
          <input type="text" placeholder="Search centres..." value={search} onChange={(e) => setSearch(e.target.value)}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-neutral-gray-light bg-neutral-bg-light">
                ...
                <th className="text-left px-4 py-3 font-medium text-neutral-gray-dark">Programs</th>
            ...
            <tbody>
              {mockCentres.map((c) => (
                <tr key={c.id} className="border-b border-neutral-gray-light last:border-0 hover:bg-neutral-bg-light/50">
                  ...
                  <td className="px-4 py-3 text-neutral-gray-dark">{c.field}</td>
                  <td className="px-4 py-3 text-neutral-gray-dark">{c.country}</td>
                  <td className="px-4 py-3">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${c.status === 'Active' ? 'bg-green-50 text-green-600' : 'bg-yellow-50 text-yellow-600'}`}>{c.status}</span>
                  </td>
                  <td className="px-4 py-3 text-neutral-gray-dark">{c.programs}</td>
