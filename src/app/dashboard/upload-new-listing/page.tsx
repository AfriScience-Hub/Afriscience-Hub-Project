...
type ListingType = 'institute' | 'scientist' | 'center' | 'innovation' | 'competition' | 'award';
...
const categories: Array<{
  ...
  { id: 'institute', title: 'Institute', description: 'University, school, or research institute', icon: Building2 },
  { id: 'scientist', title: 'Scientist / Technologist', description: 'Individual scientist, researcher, or technologist', icon: User },
  { id: 'center', title: 'Specialist Center', description: 'Specialized research or innovation center', icon: Microscope },
  { id: 'innovation', title: 'Afro-Innovation', description: 'African-born innovation or product', icon: Lightbulb },
  { id: 'competition', title: 'Competition', description: 'Science competition or challenge', icon: Trophy },
  { id: 'award', title: 'Award', description: 'Award or recognition program', icon: Award },
];

const titleMap: Record<ListingType, string> = {
...
const nameLabel: Record<ListingType, string> = {
...
function Field({
  ...
  return (
    <div className={wide ? 'lg:col-span-2' : undefined}>
      ...
      {select ? (
        <select defaultValue="" className="h-10 w-full rounded-md border border-gray-200 bg-white px-3 text-xs text-black outline-none">
...
function Textarea({
  ...
  return (
    <div>
      <label className="mb-2 block text-[11px] font-semibold text-gray-700 lg:text-xs">{label}</label>
      <textarea
        placeholder={placeholder}
  ...
  return (
    <Accordion title="Basic Profile Information" subtitle="Required" icon={User}>
      <div className="space-y-4">
        <div>
          <label className="mb-2 block text-[11px] font-semibold text-gray-700 lg:text-xs">Profile Image</label>
        ...
        {type === 'institute' && <Field label="Motto" placeholder="e.g. Knowledge for a Sustainable Future" />}
  ...
  if (type === 'scientist') {
    return (
      <Accordion title="Professional Details" icon={BriefcaseBusiness}>
        <div className="space-y-4">
          <div>
            <p className="mb-2 text-[11px] font-semibold text-gray-700 lg:text-xs">Fields of Expertise</p>
            <div className="flex flex-wrap gap-2">
              {['Biotechnology', 'Data Science', 'Environmental Science', 'Engineering', 'Medicine', 'Agriculture', 'Physics', 'Chemistry', 'Computer Science', 'Mathematics'].map((field) => (
      ...
      <div className="rounded-md border border-dashed border-gray-300 p-4">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-[1fr_auto] gap-3">
            ...
            <button className="self-end cursor-pointer rounded-md bg-red-500 px-4 py-2 text-xs font-semibold text-white hover:bg-red-600">+ Add</button>
          </div>
        </div>
      </div>
    </Accordion>
...
function Documents() {
  return (
    <Accordion title="Certifications & Documents" icon={FileText}>
      <p className="mb-3 text-[10px] text-gray-400">Upload licenses, certifications, accreditation documents, and awards (PDF or images).</p>
...
function ListingForm({ type, onChangeCategory }: { type: ListingType; onChangeCategory: () => void }) {
    ...
    <div className="space-y-4">
      <div className="flex flex-col lg:flex-row gap-4 lg:gap-0 items-center justify-between rounded-lg border border-gray-200 bg-white px-4 py-4 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-gray-400 lg:text-xs">Fill in the details below. Fields marked * are required.</p>
          </div>
        </div>
        <button
          type="button"
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <button className="w-max cursor-pointer rounded-md border border-gray-200 px-4 py-2 text-xs font-medium text-gray-700 hover:bg-gray-50">Cancel</button>
          <div className="flex gap-3">
            <button className="cursor-pointer rounded-md border border-gray-200 px-4 py-2 text-xs font-medium text-gray-700 hover:bg-gray-50">Save as Draft</button>
...
export default function UploadNewListingPage() {
  ...
  if (selectedType) {
    return <ListingForm type={selectedType} onChangeCategory={() => setSelectedType(null)} />;
  }
        ...
        {categories.map((category) => {
          ...
          return (
            <button
              ...
              className="min-h-52 cursor-pointer rounded-2xl border-2 border-gray-200 bg-white p-6 text-center transition-colors hover:border-red-400 hover:bg-red-50"
            >
              <Icon size={36} className="mx-auto mb-8 text-red-500" strokeWidth={1.8} />
              <h2 className="text-lg font-bold text-gray-950 lg:text-xl">{category.title}</h2>
              <p className="mx-auto mt-3 max-w-52 text-sm leading-relaxed text-gray-400">{category.description}</p>
            </button>
          );
        })}
      </div>
