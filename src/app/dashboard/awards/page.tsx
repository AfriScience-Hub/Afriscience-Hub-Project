...
import { Award, MapPin, Calendar, Eye, Trophy, Medal } from 'lucide-react';
...
export default function DashboardAwards() {
  ...
  const userAwards = useMemo(() => {
    return AWARD_WINNERS.filter(a =>
      (!selectedYear || a.year.toString() === selectedYear) &&
    ...
    <div>
      ...
      {userAwards.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <p className="text-slate-500 max-w-sm mt-1">No awards match the selected filters.</p>
        </div>
      ) : (
