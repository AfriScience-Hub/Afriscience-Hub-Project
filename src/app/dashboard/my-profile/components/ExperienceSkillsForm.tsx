...
import { Plus, Trash2, Upload, CheckCircle, X } from 'lucide-react';
...
export function ExperienceForm(props: ExperienceSkillsTabProps) {
    ...
    <div className="space-y-6">
      ...
      <div className="pt-4 border-t border-neutral-gray-light">
        ...
        {props.skills.length === 0 && (
          <p className="text-sm text-neutral-gray-medium">No skills added yet.</p>
        )}
      ...
      <div className="pt-4 border-t border-neutral-gray-light">
        <div className="flex items-center justify-between mb-4">
          <h4 className="text-base font-bold text-neutral-black">Languages</h4>
        ...
        {props.languages.length === 0 && (
          <p className="text-sm text-neutral-gray-medium">No languages added yet.</p>
        )}

        {props.languages.map((lang) => (
          <div key={lang.id} className="flex items-center gap-3 mb-2">
            <input type="text" value={lang.name} onChange={e => props.onLanguageChange(lang.id, 'name', e.target.value)} placeholder="Language" className="flex-1 px-3 py-2 rounded-lg border border-neutral-gray-light bg-white focus:outline-none focus:border-brand-navy-900" />
            <select value={lang.proficiency} onChange={e => props.onLanguageChange(lang.id, 'proficiency', e.target.value)} className="px-3 py-2 rounded-lg border border-neutral-gray-light bg-white focus:outline-none focus:border-brand-navy-900">
      ...
      <div className="pt-4 border-t border-neutral-gray-light">
        ...
        {props.portfolioLinks.length === 0 && (
          <p className="text-sm text-neutral-gray-medium">No work links added yet.</p>
        )}

        {props.portfolioLinks.map((link) => (
          <div key={link.id} className="flex items-center gap-3 mb-2">
            <input type="text" value={link.label} onChange={e => props.onPortfolioLinkChange(link.id, 'label', e.target.value)} placeholder="Label (e.g., GitHub)" className="w-40 px-3 py-2 rounded-lg border border-neutral-gray-light bg-white focus:outline-none focus:border-brand-navy-900" />
            <input type="url" value={link.url} onChange={e => props.onPortfolioLinkChange(link.id, 'url', e.target.value)} placeholder="https://..." className="flex-1 px-3 py-2 rounded-lg border border-neutral-gray-light bg-white focus:outline-none focus:border-brand-navy-900" />
      ...
      <div className="pt-4 border-t border-neutral-gray-light">
        ...
        <div className="flex items-center gap-3">
          ...
          {props.cvFileName && (
            <button type="button" onClick={() => props.onCvClear?.()} className="text-red-500 hover:text-red-700 cursor-pointer" title="Remove file">
              <X className="h-4 w-4" />
            </button>
          )}
