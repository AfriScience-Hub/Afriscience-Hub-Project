...
interface AfriMemesSubmissionProps {
  comp: Competition;
}
...
export function AfriMemesSubmission({ comp }: AfriMemesSubmissionProps) {
  ...
  const [editableSummary, setEditableSummary] = useState(savedApp?.memeSummary || '');
  const [savedSummary, setSavedSummary] = useState(savedApp?.memeSummary || '');
  ...
  const memeTitle = savedApp?.memeTitle || '';
  ...
  const handleSummarySave = () => {
    if (savedApp) {
      savedApp.memeSummary = editableSummary;
      sessionStorage.setItem('comp_application', JSON.stringify(savedApp));
      setSavedSummary(editableSummary);
  ...
  const handleSubmit = () => {
    if (!uploadedFile) {
      toast.error('Please upload your meme image file first.');
      return;
    }
    setSubmitting(true);
    setTimeout(() => {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm border border-neutral-gray-light">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          {memeTitle && <DetailField icon={BookOpen} label="Meme Title" value={memeTitle} />}
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm border border-neutral-gray-light">
        <h3 className="text-lg font-bold text-neutral-black mb-4 flex items-center gap-2">
          <Edit3 className="h-5 w-5 text-brand-navy-900" /> Meme Summary
        </h3>
        <textarea value={editableSummary} onChange={(e) => setEditableSummary(e.target.value)} rows={5}
        ...
        <div className="flex items-center justify-between mt-2">
          ...
          <Button size="sm" variant="outline" onClick={handleSummarySave} className="text-xs" disabled={!isSummaryChanged}>Save Summary</Button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm border border-neutral-gray-light">
        ...
        <p className="text-sm text-neutral-gray-medium mb-4">
          Upload your meme image file (JPEG, PNG or GIF).
        </p>
            ...
            <div>
              {previewUrl ? (
                <div className="relative h-36 w-52 mx-auto rounded-lg overflow-hidden mb-3">
                  <Image src={previewUrl} alt="Meme preview" fill className="object-cover" sizes="208px" />
                </div>
              ) : (
