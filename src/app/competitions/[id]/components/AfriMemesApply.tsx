...
export function AfriMemesApply({ comp }: AfriMemesApplyProps) {
  ...
  const [memeSummary, setMemeSummary] = useState('');
  ...
  const validate = () => {
    ...
    if (!memeTitle.trim()) { toast.error('Please enter a meme title.'); return false; }
    if (!memeSummary.trim()) { toast.error('Please enter a meme summary.'); return false; }
  ...
  const handlePayment = () => {
    ...
    setTimeout(() => {
      ...
      sessionStorage.setItem('comp_application', JSON.stringify({
        ...
        language: language === 'Other' ? otherLanguage : language, memeTitle, memeSummary,
        ...
        <div className="border-t border-neutral-gray-light pt-6">
          ...
          {language === 'Other' && (
            <input type="text" value={otherLanguage} onChange={(e) => setOtherLanguage(e.target.value)}
              placeholder="Specify meme language..."
