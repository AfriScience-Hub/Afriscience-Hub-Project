...
import { Trophy, FileText, User, Hash, MapPin, Calendar, Globe, BookOpen, School } from 'lucide-react';
...
export function AfriPresentationsSubmission({ comp }: Props) {
  ...
  const parentName = savedApp?.parentName || '';
  ...
  const details = [
    ...
    ...(parentName ? [{ icon: User as React.ElementType, label: 'Parent/Guardian', value: parentName }] : []),
  ...
  return (
    <div className="space-y-6">
      ...
      <UploadMediaSection submitted={submitted} uploadedFile={uploadedFile} previewUrl={previewUrl} submitting={submitting} onFileChange={handleFileChange} onSubmit={handleSubmit} onResubmit={handleResubmit} />
