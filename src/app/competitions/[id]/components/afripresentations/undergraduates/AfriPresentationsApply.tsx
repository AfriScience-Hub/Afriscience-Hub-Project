...
import {
  ...
  SummarySection, SocialHandlesSection
} from '../shared/components/ApplyFormSections';
import { DepartmentSection, MatricSection } from '../shared/components/ExtraFormSections';
import {
  ProfilePictureSection, IdCardUploadSection, GovernmentIdSection
...
export function AfriPresentationsApply({ comp }: Props) {
  ...
  const [schoolId, setSchoolId] = useState<{ file: File; preview: string } | null>(null);
  ...
  const handleSchoolIdUpload = (file: File) => {
    const reader = new FileReader();
    reader.onload = () => setSchoolId({ file, preview: reader.result as string });
    reader.readAsDataURL(file);
  };
  ...
  const validate = () => {
    ...
    if (!matricNo.trim()) { toast.error('Please enter your matriculation/registration number.'); return false; }
    if (!summary.trim()) { toast.error('Please enter a presentation summary.'); return false; }
    if (wordCount > 1000) { toast.error('Summary exceeds 1000 words.'); return false; }
    if (!profilePic) { toast.error('Please upload a profile picture.'); return false; }
    if (!schoolId) { toast.error('Please upload your school ID card.'); return false; }
  ...
  const handlePayment = () => {
    ...
    setTimeout(() => {
      ...
      sessionStorage.setItem('comp_application', JSON.stringify({
        ...
        schoolName, schoolAddress, department, matricNo, presentationSummary: summary,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm border border-neutral-gray-light space-y-6">
        ...
        <DepartmentSection department={department} onDepartmentChange={setDepartment} />
        <MatricSection matricNo={matricNo} onMatricNoChange={setMatricNo} />
        <SummarySection summary={summary} wordCount={wordCount} wordLimit={1000} onSummaryChange={setSummary} />
        <SocialHandlesSection
          linkedin={linkedin} twitter={twitter} instagram={instagram} facebook={facebook}
          onLinkedinChange={setLinkedin} onTwitterChange={setTwitter} onInstagramChange={setInstagram} onFacebookChange={setFacebook}
          title="Social Handles" subtitle=""
        />
        ...
        <div className="border-t border-neutral-gray-light pt-6">
          <IdCardUploadSection
            ...
            description='Upload "School ID Card"'
            preview={schoolId?.preview || null}
            fileName={schoolId?.file.name || null}
        ...
        <GovernmentIdSection
          ...
          preview={govtId?.preview || null}
          fileName={govtId?.file.name || null}
          onIdTypeChange={setGovtIdType}
          onOtherIdTypeChange={setOtherIdType}
          onUpload={handleGovtIdUpload}
        />
