...
import {
  CheckCircle, FileText, User, Hash, MapPin, Calendar, Briefcase,
  Edit3, Upload, Image as ImageIcon, Trophy, Clock
} from 'lucide-react';
...
interface AfriMySpaceSubmissionProps {
  comp: Competition;
}
...
export function AfriMySpaceSubmission({ comp }: AfriMySpaceSubmissionProps) {
  ...
  const [editableDescription, setEditableDescription] = useState(savedApp?.spaceDescription || '');
  const [savedDescription, setSavedDescription] = useState(savedApp?.spaceDescription || '');
  const [uploadedFile, setUploadedFile] = useState<File | null>(null);
  const [previewUrl, setPreviewUrl] = useState<string | null>(null);
  const [submitting, setSubmitting] = useState(false);
  const [submitted, setSubmitted] = useState(() => sessionStorage.getItem(`${comp.id}_submitted`) === 'true');

  const isDescriptionChanged = editableDescription !== savedDescription;
  ...
  const profession = savedApp?.profession || '';
  ...
  const handleDescriptionSave = () => {
    if (savedApp) {
      savedApp.spaceDescription = editableDescription;
      sessionStorage.setItem('comp_application', JSON.stringify(savedApp));
      setSavedDescription(editableDescription);
  ...
  const handleSubmit = () => {
    if (!uploadedFile) {
      toast.error('Please upload your workspace photo first.');
      return;
    }
    setSubmitting(true);
    setTimeout(() => {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm border border-neutral-gray-light">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          {profession && <DetailField icon={Briefcase} label="Profession" value={profession} />}
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm border border-neutral-gray-light">
        <h3 className="text-lg font-bold text-neutral-black mb-4 flex items-center gap-2">
          <Edit3 className="h-5 w-5 text-brand-navy-900" /> Space Description
        </h3>
        <textarea value={editableDescription} onChange={(e) => setEditableDescription(e.target.value)} rows={5}
        ...
        <div className="flex items-center justify-between mt-2">
          ...
          <Button size="sm" variant="outline" onClick={handleDescriptionSave} className="text-xs" disabled={!isDescriptionChanged}>Save Summary</Button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm border border-neutral-gray-light">
        ...
        <p className="text-sm text-neutral-gray-medium mb-4">
          Upload your workspace photo (JPEG, PNG or GIF).
        </p>

        <div className={cn("border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-colors",
          ...
          {uploadedFile ? (
            <div>
              {previewUrl ? (
                <div className="relative h-36 w-52 mx-auto rounded-lg overflow-hidden mb-3">
                  <Image src={previewUrl} alt="Workspace preview" fill className="object-cover" sizes="208px" />
                </div>
              ) : (
            ...
            <div>
              <ImageIcon className="h-12 w-12 text-neutral-gray-light mx-auto mb-3" />
              <p className="font-medium text-neutral-gray-dark">Click to upload or drag and drop</p>
              <p className="text-xs text-neutral-gray-medium mt-1">JPEG, PNG or GIF</p>
            </div>
          )}
        </div>
        ...
        <div className="mt-6">
          ...
          <p className="text-xs text-neutral-gray-medium text-center mt-3">
            You can resubmit a more recent photo to overwrite this submission until the deadline.
          </p>
        </div>
      </div>
