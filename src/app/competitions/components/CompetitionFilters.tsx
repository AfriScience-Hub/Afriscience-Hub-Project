...
import { COMPETITION_TYPES, AFRICAN_COUNTRIES } from '../data';
...
export function CompetitionFilters({
  ...
  toggleSection, resetFilters, activeFilterCount, categories, currentYear,
    ...
    <aside className={cn("w-full lg:w-72 flex-shrink-0 space-y-6 transition-all duration-300", showFilters ? "block" : "hidden lg:block")}>
      <div className="rounded-xl border border-neutral-gray-light bg-white p-5 shadow-sm lg:sticky lg:top-20 max-h-[calc(100vh-6rem)] overflow-y-auto custom-scrollbar">
        ...
        <div className="space-y-0 divide-y divide-neutral-gray-light">
                ...
                {!selectedType ? (
                  ...
                  <div className="space-y-2">{categories.map(cat => (
                    <label key={cat} className="flex items-center gap-2 cursor-pointer group">
                      <input type="radio" name="competition-category" checked={selectedCategory === cat} onChange={() => setSelectedCategory(cat)} className="border-neutral-gray-light text-brand-red-600 focus:ring-brand-red-600" />
          ...
          {/* Year */}
          <div className="py-4">
            <div className="flex items-center justify-between text-sm font-bold text-neutral-black"><span>Year</span></div>
            <div className="mt-3 animate-in fade-in duration-200">
              <div className="flex items-center gap-2 bg-brand-red-50 text-brand-red-700 px-3 py-2 rounded-lg border border-brand-red-100">
